...
def sample(model,
    ...
    for k in range(steps):
        ...
        lat_logits, lon_logits, sog_logits, cog_logits, deltaTime_logits = \
            torch.split(logits, (model.lat_size, model.lon_size, model.sog_size, model.cog_size, model.deltaTime_size), dim=-1)
        ...
        if top_k is not None:
            ...
            deltaTime_logits = utils.top_k_logits(deltaTime_logits, top_k)
        ...
        if sample:
            ...
            deltaTime_ix = torch.multinomial(deltaTime_probs, num_samples=1)
        else:
            ...
            _, deltaTime_ix = torch.topk(deltaTime_probs, k=1, dim=-1)

        #ix = torch.cat((lat_ix, lon_ix, sog_ix, cog_ix), dim=-1)
        ix = torch.cat((lat_ix, lon_ix, sog_ix, cog_ix, deltaTime_ix), dim=-1)
...
class Trainer:
    ...
    def __init__(self, model, train_dataset, test_dataset, config, savedir=None, device=torch.device("cpu"), aisdls={},
        ...
        self.aisdls = aisdls
        logging.info(f"aisdls has %s",self.aisdls)

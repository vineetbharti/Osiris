...
class TrAISformer(nn.Module):
    ...
    def __init__(self, config, partition_model = None):
        ...
        self.deltaTime_size = config.deltaTime_size
        ...
        self.n_deltaTime_embd = config.n_deltaTime_embd
        self.register_buffer(
            "att_sizes", 
            #torch.tensor([config.lat_size, config.lon_size, config.sog_size, config.cog_size]))
            torch.tensor([config.lat_size, config.lon_size, config.sog_size, config.cog_size, config.deltaTime_size]))
        self.register_buffer(
            "emb_sizes", 
            #torch.tensor([config.n_lat_embd, config.n_lon_embd, config.n_sog_embd, config.n_cog_embd]))
            torch.tensor([config.n_lat_embd, config.n_lon_embd, config.n_sog_embd, config.n_cog_embd, config.n_deltaTime_embd]))
        ...
        if hasattr(config,"lat_min"): # the ROI is provided.
            ...
            self.sog_range = 30.
            self.deltaTimeRange = config.deltaTime_size
        ...
        self.deltaTime_emb = nn.Embedding(self.deltaTime_size, config.n_deltaTime_embd)
    ...
    def to_indexes(self, x, mode="uniform"):
        ...
        Args:
            ...
            x: a Tensor of size (batchsize, seqlen, 5). x has been truncated 
                to [0,1).
...
#             pdb.set_trace()
    ...
    def forward(self, x, masks = None, with_targets=False, return_loss_tuple=False):
        ...
        deltaTime_embeddings = self.deltaTime_emb(inputs[:,:,4])      
        #token_embeddings = torch.cat((lat_embeddings, lon_embeddings, sog_embeddings, cog_embeddings),dim=-1)
        token_embeddings = torch.cat((lat_embeddings, lon_embeddings, sog_embeddings, cog_embeddings, deltaTime_embeddings),dim=-1)
        ...
        lat_logits, lon_logits, sog_logits, cog_logits, deltaTime_logits =\
            torch.split(logits, (self.lat_size, self.lon_size, self.sog_size, self.cog_size, self.deltaTime_size), dim=-1)
        # Calculate the loss
        loss = None
        loss_tuple = None
        if targets is not None:
            ...
            cog_loss = F.cross_entropy(cog_logits.view(-1, self.cog_size), 
                                       targets[:,:,3].view(-1), 
                                       reduction="none").view(batchsize,seqlen)
            deltaTime_loss = F.cross_entropy(deltaTime_logits.view(-1, self.deltaTime_size), 
                                       targets[:,:,4].view(-1), 
                                       reduction="none").view(batchsize,seqlen)
            ...
            if self.blur:
                ...
                deltaTime_probs = F.softmax(deltaTime_logits, dim=-1)
                ...
                for _ in range(self.blur_n):
                    ...
                    blurred_deltaTime_probs = self.blur_module(deltaTime_probs.reshape(-1,1,self.deltaTime_size)).reshape(deltaTime_probs.shape)
                    ...
                    blurred_cog_loss = F.nll_loss(blurred_cog_probs.view(-1, self.cog_size),
                                                  targets[:,:,3].view(-1),
                                                  reduction="none").view(batchsize,seqlen)
                    blurred_deltaTime_loss = F.nll_loss(blurred_deltaTime_probs.view(-1, self.deltaTime_size),
                                                  targets[:,:,4].view(-1),
                                                  reduction="none").view(batchsize,seqlen)
                    ...
                    deltaTime_loss += self.blur_loss_w*blurred_deltaTime_loss

                    lat_probs = blurred_lat_probs
                    lon_probs = blurred_lon_probs
                    sog_probs = blurred_sog_probs
                    cog_probs = blurred_cog_probs
                    deltaTime_probs = blurred_deltaTime_probs
                    

            loss_tuple = (lat_loss, lon_loss, sog_loss, cog_loss, deltaTime_loss)
            loss = sum(loss_tuple)
